...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=org.gnucash.android.devel"
...
app-original.apk : 
	...
	$(GW) -PjacocoEnabled=false assembleDevelopment && \
	mv app/build/outputs/apk/development/debug/GnucashAndroid_v2.4.1-dev4_r.apk app-debug.apk && \
...
app-androidTest.apk :
	...
	mv /Volumes/TOSHIBA/carving/apps-src/gnucash-android/app/build/outputs/apk/androidTest/development/debug/app-development-debug-androidTest.apk app-androidTest-unsigned.apk && \
...
$(ESPRESSO_TESTS) : app-androidTest.apk app-instrumented.apk
	...
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c org.gnucash.android.devel))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear org.gnucash.android.devel; \
	else \
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) org.gnucash.android.devel.test/org.gnucash.android.test.ui.util.GnucashAndroidTestRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces org.gnucash.android.devel ./traces/$(TEST_NAME) force-clean
